...
export const blogPosts: BlogPost[] = EN_POSTS.map((post) => {
  const hebrewOverlay = requireHebrewPost(post);

  return {
    ...post,
    title: { en: post.title, he: hebrewOverlay.title },
    excerpt: { en: post.excerpt, he: hebrewOverlay.excerpt },
    content: { en: post.content, he: hebrewOverlay.content },
  };
});
...
export const getRecentPosts = (limit = 3): BlogPost[] =>
  [...blogPosts].sort(byNewest).slice(0, limit);
