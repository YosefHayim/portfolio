...
const fetchUserCount = async (id: string): Promise<number | null> => {
  ...
  try {
    ...
    logger.warn('chrome_extension_users_fetch_failed', {
      extensionId: id,
...
export const chromeExtensionsRouter: Router = Router();

chromeExtensionsRouter.get(
  '/:id/users',
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (users === null) {
      ...
      return res.status(502).json({ success: false, error: 'Could not read user count' });
    }
